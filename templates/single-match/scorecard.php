...
$runs          = maybe_unserialize( get_post_meta( $post->ID, '_wpcm_match_runs', true ) );
$extras        = maybe_unserialize( get_post_meta( $post->ID, '_wpcm_match_extras', true ) );
$wickets       = maybe_unserialize( get_post_meta( $post->ID, '_wpcm_match_wickets', true ) );
$overs         = maybe_unserialize( get_post_meta( $post->ID, '_wpcm_match_overs', true ) );
if ( $club == $home_club ) {
	$runs    = $runs['home'];
	$extras  = $extras['home'];
			...
			<table class="wpcm-lineup-table">
				<thead>
					<tr>
						...
						<?php
						foreach ( $batting_stats as $key => $val ) {
			...
			<table class="wpcm-lineup-table">
				<thead>
					<tr>
						...
						<?php
						foreach ( $bowling_stats as $key => $val ) {
							if ( ! in_array( $key, wpcm_exclude_keys() ) && get_option( 'wpcm_show_stats_' . $key ) == 'yes' && get_option( 'wpcm_match_show_stats_' . $key ) == 'yes' ) {
